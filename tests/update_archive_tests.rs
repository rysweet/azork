...
fn extract_rejects_parent_traversal_entry() {
    ...
    let archive = make_targz(&[("../azork_escaped_marker", b"pwned")]);
...
fn extract_rejects_absolute_path_entry() {
    let dir = tempdir();
    let archive = make_targz(&[("/tmp/azork_abs_marker", b"pwned")]);
    let result = extract_binary(&archive, "azork", dir.path());
...
fn tempdir() -> TempDir {
    ...
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let path = std::env::temp_dir().join(format!("azork-archive-test-{nanos}-{:p}", &nanos as *const _));
