...
| 10 | Dungeon Crawler SVG scrubbing corrupted markup for secret-shaped resource names | `redact_bearer_tokens` in `src/secrets.rs` matched the substring `"bearer"` case-insensitively with no preceding word-boundary check, then consumed everything up to the next whitespace as the "token" to redact. A resource whose name merely *contained* `bearer` (e.g. an App Service named `torchBearer`) was enough to trigger a false-positive match, and because SVG/HTML markup has no interior whitespace (`</title><rect x="570" .../>`), the scrubber swallowed the closing `</title>` tag and the following tag's name/attributes as part of the "token," emitting invalid markup such as `<title>torchBearer ***REDACTED*** x="570" y="1574" .../>` — dropping the `</title>` close tag, the `<rect` tag name, and the `icon icon-app-service` resource-type class in the process. | Medium | `src/secrets.rs::redact_bearer_tokens` (invoked from `scrub`, which `render.rs` calls on every resource/room label before writing Dungeon Crawler Mode's SVG/HTML map) | **Fixed** — `redact_bearer_tokens` now requires a real word boundary immediately before a `bearer`/`Bearer` match (start of string, or a non-alphanumeric byte) before treating it as an authorization-header prefix, so a resource name like `torchBearer` is left untouched — it is not a `Bearer <token>` header, just a name that happens to contain the letters. Text preceding a *rejected* candidate match (i.e. one that fails the boundary check) is now copied verbatim to the output buffer instead of being silently dropped, closing the swallowed-tag bug regardless of what markup happens to sit next to the false-positive substring. Unit-tested: a regression test reproduces the exact `<title>`/`<rect>` snippet from [issue #34](https://github.com/rysweet/azork/issues/34) and asserts byte-for-byte that `</title>`, `<rect`, every attribute, and `icon icon-app-service` survive scrubbing unchanged; a second test confirms a real tag-adjacent token (`Bearer abc123</title>`) is still fully redacted, so the boundary fix does not reintroduce under-redaction. Closes [issue #34](https://github.com/rysweet/azork/issues/34). |
...
| `cargo test --locked` | ✅ Pass — 138 lib tests (366 total across the lib + integration/parser/world/update/memory/oit/dungeon test binaries), all green (includes the `secrets.rs` regression tests for Finding #10) |
...
- **Sibling secret detectors share the same class of bug the Finding #10 fix
  addressed for `redact_bearer_tokens`.** The word-boundary and
  verbatim-copy-on-rejected-match fix was applied specifically to the
  `bearer `-prefix path in `src/secrets.rs`. Other pattern matchers in the
  same file (key/value pairs, connection strings, SAS query strings,
  JWT-shaped tokens) were reviewed and not found to reproduce the exact
  swallowed-tag failure mode, but they were not exhaustively boundary-audited
  as part of this fix. A follow-up audit of all `src/secrets.rs` matchers for
  the same "false-positive substring match consumes trailing markup" pattern
  is recommended; see the tracking issue referenced from
  [issue #34](https://github.com/rysweet/azork/issues/34).
