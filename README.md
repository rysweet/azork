...
Here's a map generated by `azork crawl --backend mock --mock-size 40x13 --serve`
against the built-in, deterministic offline mock backend (a synthetic tenant
of 40 resource groups and 520 resources), reflecting the adaptive room
sizing, spaced-out corridors, and dungeon decorations (border, torches,
treasure chest, dragon). The mock backend is used here instead of a real
subscription because it's reproducible and legible for documentation, and
requires no live Azure access:
