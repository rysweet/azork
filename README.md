...
**corridors and doors**, resources appear inside their room as **original,
hand-authored resource-type icons** (bundled offline, not hotlinked, and not
Microsoft's copyrighted icon artwork), and shared regions/relationships
become the corridors between rooms — drawn on a parchment-and-grid
background so it reads as a dungeon map, not a graph.
...
shows its resource-type icon (an original AzZork glyph, not Microsoft's icon
artwork), a deep link straight to that resource's page in the Azure portal,
...
drawn with its original resource-type icon, and corridors with doors connect
resource groups that share regions or relationships.*
...
an original, hand-authored monochrome icon for its type (storage account,
virtual network, virtual machine, key vault, Cosmos DB, and more) — see
